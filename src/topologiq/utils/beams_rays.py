...
class RayBeam:
    ...
    def intersected_by(self, that):
        ...
        if relative_orientation == 0:
            ...
            # The source of the other beam must lie in the quadrant generated by
            # > Sigma = linear combination of self.direction and -that.direction with non-negative scalars
            basis = self.direction - that.direction
            joint = np.sign(sigma) * np.sign(basis)
            # Need to check that there is no 0 in the basis with corresponding non-zero in sigma
            intra = np.all( (basis != 0) | (sigma == 0) )
            poslc = np.count_nonzero(joint < 0) == 0
            # and np.count_nonzero(np.sign(sigma) == np.sign(basis)) < 3
            report += f"> Sigma : {sigma}\n"
            report += f"> Basis : {basis}\n"
            report += f"> Cross : {cross}\n"
            report += f"> Intra : {intra}\n"
            report += f"> PosLC : {poslc}\n"
            report += f"> Joint : {joint}\n"
            intersecting_rays = intra and poslc
        else:
            # Beams are in identical or opposite directions
            # The source of the other beam must lie on the same line as the direction of this beam
